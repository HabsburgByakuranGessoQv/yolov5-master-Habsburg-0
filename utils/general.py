...
def check_file(file, suffix=''):
    ...
    if os.path.isfile(file) or not file:  # exists
        ...
        assert 'clearml' in sys.modules, "ClearML is not installed, so cannot use ClearML datasets. Try running 'pip install clearml'."
        return file
    else:  # search
        files = []
...
def non_max_suppression(
    ...
    YOLOv5中使用NMS算法来移除一些网络模型预测时生成的多余的检测框，
    该算法的核心思想是指搜索局部得分最大值预测并移除与局部最大值预测框重叠度超过一定阈值的检测框，
    需要注意的是，NMS算法对所有待检测目标类别分别执行，即为不同类别的检测框即使有重叠也不会被移除。
    Returns:
