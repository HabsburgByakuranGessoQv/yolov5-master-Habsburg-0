...
def exist_objs(list_1, list_2, new_box_iou_limit=0.4):
    ...
    def judge_iou_limit():
        ...
        if new_box_area / (box_area) >= new_box_iou_limit:
    ...
    for object_box in list_2:
        ...
        if s_xmin <= xmin < s_xmax and s_ymin <= ymin < s_ymax:  # 目标点的左上角在切图区域中
            ...
            if xmax > s_xmax and s_ymin < ymax <= s_ymax:  # 目标点的右下角在切图区域右方
                ...
                ymax_new = ymax - s_ymin
                judge_iou_limit()
    return_objs = pd.DataFrame(return_objs).drop_duplicates().values.tolist()
    return return_objs
...
def write_label(labelPath, labelBoxes):
    with open(labelPath, mode='w') as f:
        for box in labelBoxes:
            for i in range(len(box)):
                if i == 0:
                    f.write(str(int(box[i])))
                else:
                    f.write(str(box[i]))
                if i == len(box) - 1:
                    f.write('\n')
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with open(os.path.join(self.image_path, image_name), 'rb') as f:
            # 这样读就是二进制的
            f = f.read()
        # 这句 就是补全数据的
        f = f + B'\xff' + B'\xd9'
        ...
        # image = Image.open(os.path.join(self.image_path, image_name)).convert('RGB')  # 读取到的是RGB， C, H, W遇到有损坏的数据，该方法报错
        #         print(image_name)
...
def toYolo(box, imageWidth, imageHeight):
    ...
    return int(box[0]), round(center_x, 5), round(center_y, 5), round(width, 5), round(height, 5)
...
class Crop():
    ...
    def inputLabel(self, labelPath, label_names=None, coordinates='yolo'):
...
class slidingWindowCrop(Crop):
    ...
    def saveSubImage(self, index, imgs_save_path, overlap=0.5, resize=None, ):
        """通过索引保存子图"""
        # 如果不存在文件夾则创建
    ...
    def saveSubImageAndTxt(self, index, imgs_save_path, labels_save_path, overlap=0.5,
                           resize=None, new_box_iou_limit=0.35, coordinates='yolo', save_all=False):
        ...
        for num, (sub_image, exiset_obj_list) in enumerate(zip(slice_images, exiset_obj_lists)):
            # if exiset_obj_list and len(exiset_obj_list)!=len(np.unique(exiset_obj_list,axis = 1)):
            #     print('double',len(exiset_obj_list), len(np.unique(exiset_obj_list,axis = 1)))
            # 子图位置编号
            sub_row = (num) // ncol
            sub_col = (num) % ncol

            n_save_imgs += 1
            path_image = os.path.join(imgs_save_path, image_name.split('.')[0] + f'_{sub_row}' + f'_{sub_col}_oir.jpg')
            if len(exiset_obj_list) != 0 or save_all:
            ...
            path_label = os.path.join(labels_save_path,
                                      image_name.split('.')[0] + f'_{sub_row}' + f'_{sub_col}_oir.txt')
            # 如果已存在该子图名称文件，可能会重复写入，因此移除来重新写入
            if os.path.exists(path_label):
                os.remove(path_label)

            if len(exiset_obj_list) == 0 and save_all:
                # 保存空文件
                print('save:', path_image, '  ', path_label)
                save_txt(path_label, [])
            elif len(exiset_obj_list) != 0:
                print('save:', path_image, '  ', path_label)
...
def randomCropPosition(labels, Width, Height, subWidth, subHeight):
    label, xmin, ymin, xmax, ymax = labels[:5]
    ...
    def saveSubImageAndTxt(self, index, imgs_save_path, labels_save_path, coordinates='yolo',
        ...
        for num, (sub_image, exiset_obj_list) in enumerate(zip(images, exiset_obj_lists)):
            if exiset_obj_list:
                ...
                num_ = n_save_imgs
                while True:
                    path_image = os.path.join(imgs_save_path, image_name.split('.')[0] + f'_{num_ - 1}_oir.jpg')
                ...
                if resize:
                    ...
                    transforms.ToPILImage()(sub_image).save(path_image)
                # 保存子图相对应labels的txt文件到指定路径
                path_label = os.path.join(labels_save_path, image_name.split('.')[0] + f'_{num_ - 1}_oir.txt')
                # print('save:', path_image, '  ', path_label)
                # 如果已存在该子图名称文件，可能会重复写入，因此移除来重新写入
                if os.path.exists(path_label):
                    os.remove(path_label)
                # 判断标签重复情况
                if len(pd.DataFrame(exiset_obj_list).drop_duplicates()) != len(exiset_obj_list):
                    print('double', exiset_obj_list, len(pd.DataFrame(exiset_obj_list).drop_duplicates()),
                          len(exiset_obj_list))
                    print('*' * 100)
                for box in exiset_obj_list:
                    save_txt(path_label,
...
if __name__ == '__main__':
    ...
    label_path_main = r'D:\StuData\numbers\labels'
    save_img_main = r'D:\StuData\numbers\new_2\images'
    save_label_main = r'D:\StuData\numbers\new_2\labels'
    size_main = (640, 480)
