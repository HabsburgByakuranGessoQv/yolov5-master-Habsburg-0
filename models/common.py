...
class Conv(nn.Module):
	...
	# 这个函数是整个网络中最基础的组件，由 卷积层 + BN层 + 激活函数 组成
	...
	def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
		...
		"""
		super().__init__()
		self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False) # Conv
		self.bn = nn.BatchNorm2d(c2) # Bn
		# 目的是根据不同的条件，给 self.act 赋值一个合适的激活函数，如果没有指定激活函数或者指定的激活函数不是 nn.Module 的实例，则默认为恒等映射函数 nn.Identity()
		self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity() # 一个空的恒等映射函数 nn.Identity()

	def forward(self, x):
		return self.act(self.bn(self.conv(x)))

	def forward_fuse(self, x):
		# 特殊函数 forward_fuse ，这是一个前向加速推理模块，在前向传播过程中，通过融合 Conv + BN 层，达到加速推理的作用，一般用于测试或验证阶段
...
class Bottleneck(nn.Module):
	# Standard bottleneck
	# shortcut = False : input -> conv -> conv -> ...
	# shortcut = True : (input -shortcut-) + (input -> conv -> conv) -> ...
...
class C3(nn.Module):
	...
	"""
	这个模块是一种简化的BottleneckCSP，因为除了Bottleneck部分只有3个卷积，可以减少参数，所以取名C3。
	而原作者之所以用C3来代替BottleneckCSP也是有原因的，作者原话：
...
class SPP(nn.Module):
	...
	经典的空间金字塔池化模块首先将输入的卷积特征分成不同的尺寸，
	然后每个尺寸提取固定维度的特征，最后将这些特征拼接成一个固定的维度
	"""
...
class Focus(nn.Module):
	...
	# 为了减少浮点数和提高速度，而不是增加featuremap的，本质就是将图像进行切片，类似于下采样取值，将原图像的宽高信息切分，聚合到channel通道中
...
class Contract(nn.Module):
	# Contract函数改变输入特征的shape，将feature map的 w 和 h 维度（缩小）的数据收缩到channel维度上（放大）。
	# 如：x(1,64,80,80) to x(1,256,40,40)
...
class Expand(nn.Module):
	# Expand函数也是改变输入特征的shape，不过与Contract的相反，是将channel维度（变小）的数据扩展到 W 和 H 维度（变大）。
	# 如：x(1,64,80,80) to x(1,16,160,160)
...
class Concat(nn.Module):
	# 这个函数是将自身（a list of tensors）按照某个维度进行concat，通常用来合并前后两个feature map
...
class Classify(nn.Module):
	...
	# 这是一个二级分类模块。什么是二级分类模块？
	# 比如做车牌识别，先识别出车牌，如果相对车牌上的字进行识别，就需要二级分类进一步检测
	def __init__(self,
				 c1,
				 c2,
				 k=1,
...
# start--------------------------CBAMC3 Model------------------------------
# ----------ChannelAttention + SpatialAttention + C3 = CBAMC3 -------------
# -------------------------------C3 ---> CBAMC3----------------------------
# *************************************************************************
# start--------------------------Conv_CBAM Model---------------------------
# ----------ChannelAttention + SpatialAttention = Conv_CBAM ---------------
# -------------------------------Conv ---> Conv_CBAM-----------------------
